...
//------------------------------------------------------------------------------
// ISR variables.
volatile static uint8_t* TWI_buf;
volatile static size_t TWI_nbytes;
volatile static uint8_t TWI_option;
...
//  TWI_state = TWSR;
  ...
  // start
  if (TWSR != TWI_REP_START && TWSR != TWI_START) {
    TWCR = (1<<TWEN)|(0<<TWIE)|(1<<TWINT)|
